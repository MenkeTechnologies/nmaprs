...
fn ttl_guess(ttl: u8) -> u8 {
    if ttl <= 32 { 32 } else if ttl <= 64 { 64 } else if ttl <= 128 { 128 } else { 255 }
}
...
fn quirks_str(reserved: u8, flags: u8, urg: u16) -> String {
    let mut s = String::new();
    if reserved != 0 { s.push('R'); }
    if urg != 0 && flags & TcpFlags::URG == 0 { s.push('U'); }
    s
}

fn seq_relation(resp_seq: u32, probe_ack: u32) -> String {
    if resp_seq == 0 { "Z".into() }
    else if resp_seq == probe_ack { "A".into() }
    else if resp_seq == probe_ack.wrapping_add(1) { "A+".into() }
    else { "O".into() }
}

fn ack_relation(resp_ack: u32, probe_seq: u32) -> String {
    if resp_ack == 0 { "Z".into() }
    else if resp_ack == probe_seq { "S".into() }
    else if resp_ack == probe_seq.wrapping_add(1) { "S+".into() }
    else { "O".into() }
}
...
fn cc_str(flags: u8) -> String {
    ...
    match (ece, cwr) {
        (true, false) => "Y", (false, false) => "N", (true, true) => "S", _ => "O",
    }.into()
}

fn rd_value(data: &[u8]) -> String {
    if data.is_empty() { "0".into() } else { format!("{:X}", crc32fast::hash(data)) }
}
...
pub fn tcp_options_fingerprint(tcp: &TcpPacket<'_>, _mss_hint: u16) -> Result<String, ()> {
    ...
    while !q.is_empty() && out.len() < 240 {
        let opcode = q[0];
        if opcode == 0 { out.push('L'); break; }
        if opcode == 1 { out.push('N'); q = &q[1..]; continue; }
        if q.len() < 2 { return Err(()); }
        let len = q[1] as usize;
        if len < 2 || q.len() < len { return Err(()); }
        match opcode {
            2 => { if len < 4 { return Err(()); }
                   let mss = u16::from_be_bytes([q[2], q[3]]);
                   let _ = write!(&mut out, "M{:X}", mss); }
            3 => { if len < 3 { return Err(()); }
                   let _ = write!(&mut out, "W{:X}", q[2]); }
            4 => { out.push('S'); }
            8 => { if len < 10 { return Err(()); }
                   out.push('T');
...
fn read_timestamp(tcp: &TcpPacket<'_>) -> u32 {
    let hdr = tcp.packet();
    let data_off = tcp.get_data_offset() as usize * 4;
    if data_off <= 20 || hdr.len() < data_off { return 0; }
    let mut q = &hdr[20..data_off];
    while !q.is_empty() {
        let op = q[0];
        if op == 0 { break; }
        if op == 1 { q = &q[1..]; continue; }
        if q.len() < 2 { break; }
        let len = q[1] as usize;
        if len < 2 || q.len() < len { break; }
        if op == 8 && len >= 10 {
...
fn recv_tcp_reply(
    rx: &mut TransportReceiver, target: Ipv4Addr, our_port: u16, timeout: Duration,
    ...
    loop {
        ...
        let Some(ip) = recv_ipv4_with_timeout(rx, wait)? else { continue };
        if ip.get_source() != target { continue; }
        if ip.get_next_level_protocol() != IpNextHeaderProtocols::Tcp { continue; }
        let Some(tcp) = TcpPacket::new(ip.payload()) else { continue };
        if tcp.get_destination() != our_port { continue; }
        ...
        return Ok(Some(TcpReply {
            ipid, ttl, df, window, seq, ack_num, flags, reserved, urg_ptr,
            options_fp, tsval, data, recv_time: Instant::now(),
        }));
    }
}

fn recv_icmp_echo_reply(
    rx: &mut TransportReceiver, target: Ipv4Addr, expected_id: u16, timeout: Duration,
    ...
    loop {
        ...
        let Some(ip) = recv_ipv4_with_timeout(rx, wait)? else { continue };
        if ip.get_source() != target { continue; }
        if ip.get_next_level_protocol() != IpNextHeaderProtocols::Icmp { continue; }
        let p = ip.payload();
        if p.len() < 8 || p[0] != 0 { continue; }
        let id = u16::from_be_bytes([p[4], p[5]]);
        if id != expected_id { continue; }
        return Ok(Some(IcmpEchoReply {
            ipid: ip.get_identification(), ttl: ip.get_ttl(),
            df: ip.get_flags() & 0b010 != 0, code: p[1],
        }));
    }
}

fn recv_icmp_unreachable(
    rx: &mut TransportReceiver, target: Ipv4Addr, expected_udp_dport: u16, timeout: Duration,
    ...
    loop {
        ...
        if ip.get_next_level_protocol() != IpNextHeaderProtocols::Icmp { continue; }
        let p = ip.payload();
        if p.len() < 36 { continue; } // 8 ICMP hdr + 20 embedded IP + 8 embedded UDP
        if p[0] != 3 || p[1] != 3 { continue; }
        let embedded = &p[8..];
        let ihl = (embedded[0] & 0x0f) as usize * 4;
        if embedded.len() < ihl + 4 { continue; }
        let udp_dport = u16::from_be_bytes([embedded[ihl + 2], embedded[ihl + 3]]);
        if udp_dport != expected_udp_dport { continue; }
        ...
        let eudp_data = if embedded.len() > ihl + 8 { embedded[ihl + 8..].to_vec() } else { Vec::new() };
        return Ok(Some(IcmpUnreachReply {
            ttl: ip.get_ttl(), df: ip.get_flags() & 0b010 != 0,
            ip_total_len: ip.get_total_length(), unused,
            embedded_ip_total_len: eip_total, embedded_ip_id: eip_id,
            embedded_ip_cksum: eip_ck, embedded_ip_cksum_computed: eip_ck_computed,
            embedded_udp_cksum: eudp_ck, embedded_udp_data: eudp_data,
        }));
    }
}
...
fn send_tcp_probe(
    tx: &mut TransportSender, src: Ipv4Addr, dst: Ipv4Addr, sport: u16, dport: u16,
    seq: u32, ack: u32, reserved: u8, flags: u8, window: u16, urg: u16,
    df: bool, opts: &[u8], ttl: u8, ip_id: u16,
) -> io::Result<()> {
...
fn send_udp_probe(
    tx: &mut TransportSender, src: Ipv4Addr, dst: Ipv4Addr, sport: u16, dport: u16,
    data: &[u8], ttl: u8, df: bool, ip_id: u16,
) -> io::Result<u16> {
...
fn send_icmp_echo(
    tx: &mut TransportSender, src: Ipv4Addr, dst: Ipv4Addr,
    code: u8, id: u16, seq: u16, data_len: usize,
    ttl: u8, df: bool, tos: u8, ip_id: u16,
) -> io::Result<()> {
    ...
    }
    {
        ...
        if df { ip.set_flags(Ipv4Flags::DontFragment); }
...
fn build_ip_header(buf: &mut [u8], total_len: u16, id: u16, df: bool, ttl: u8, proto: u8, src: Ipv4Addr, dst: Ipv4Addr) {
...
fn classify_ipid(ipids: &[u16]) -> String {
    if ipids.len() < 2 { return "O".into(); }
    if ipids.iter().all(|&v| v == 0) { return "Z".into(); }

    let mut diffs = Vec::with_capacity(ipids.len() - 1);
    for i in 1..ipids.len() {
        let cur = ipids[i] as u32;
        let prev = ipids[i - 1] as u32;
        let d = if cur >= prev { cur - prev } else { 0x10000 + cur - prev };
        diffs.push(d);
    }

    if diffs.iter().all(|&d| d == 0) { return "Z".into(); }
    if diffs.iter().any(|&d| d > 20000) { return "RD".into(); }
    ...
    if bi {
        ...
        if swap_diffs.iter().all(|&d| d <= 20000) { return "BI".into(); }
    }

    let avg: f64 = diffs.iter().map(|&d| d as f64).sum::<f64>() / diffs.len() as f64;
    if avg < 1.0 { return "Z".into(); }
    let var: f64 = diffs.iter().map(|&d| { let x = d as f64 - avg; x * x }).sum::<f64>() / diffs.len() as f64;
    if var.sqrt() / avg < 0.1 && avg < 5000.0 { return "I".into(); }
    "RI".into()
}

fn classify_ts(timestamps: &[u32], times: &[Instant]) -> String {
    if timestamps.len() < 2 { return "U".into(); }
    if timestamps.iter().all(|&t| t == 0) { return "0".into(); }

    for i in 1..timestamps.len() {
        if timestamps[i] < timestamps[i - 1] {
            if timestamps[i - 1] - timestamps[i] > 1000 { return "U".into(); }
        }
    }
    ...
    if time_diff < 0.001 { return "U".into(); }
    let freq = ts_diff / time_diff;

    if freq < 0.5 { return "0".into(); }
    if freq < 5.66 { return "1".into(); }
    if (85.0..350.0).contains(&freq) { return "7".into(); }
    if (700.0..1500.0).contains(&freq) { return "8".into(); }
    format!("{:X}", freq.round() as u32)
}

fn shared_ipid_seq(tcp_ipids: &[u16], icmp_ipids: &[u16]) -> String {
    if tcp_ipids.len() < 2 || icmp_ipids.is_empty() { return "O".into(); }
    let ti = classify_ipid(tcp_ipids);
    let ii = classify_ipid(icmp_ipids);
    if ti != "I" || ii != "I" { return "O".into(); }
    let last_tcp = *tcp_ipids.last().unwrap() as i32;
    let first_icmp = icmp_ipids[0] as i32;
    if (first_icmp - last_tcp).unsigned_abs() < 256 { "S".into() } else { "O".into() }
}
...
fn build_seq_test(
    seq_nums: &[u32], seq_times: &[Instant], timestamps: &[u32],
    tcp_open_ipids: &[u16], tcp_closed_ipids: &[u16], icmp_ipids: &[u16],
) -> HashMap<String, String> {
    let n = seq_nums.len();
    let mut diffs = Vec::with_capacity(n - 1);
    for i in 1..n { diffs.push(mod_diff_u32(seq_nums[i], seq_nums[i - 1])); }
    let gcd = gcd_many(&diffs);
    let mut seq_rates = Vec::with_capacity(n - 1);
    for i in 1..n {
        let dt = seq_times[i].saturating_duration_since(seq_times[i - 1]).as_micros().max(1);
        seq_rates.push(diffs[i - 1] as f64 * 1_000_000.0 / dt as f64);
    }
    let avg: f64 = seq_rates.iter().sum::<f64>() / seq_rates.len().max(1) as f64;
    let mut isr = if avg > 0.0 { (avg.log2() * 8.0).round() as u32 } else { 0 };
    if gcd == 0 { isr = 0; }

    let div_gcd = if gcd > 9 { gcd as f64 } else { 1.0 };
    let mean_normed = avg / div_gcd;
    let var: f64 = seq_rates.iter().map(|r| { let t = r / div_gcd - mean_normed; t * t }).sum::<f64>()
        / (seq_rates.len().saturating_sub(1).max(1)) as f64;
    let sd = var.sqrt();
    let sp = if sd <= 1.0 { 0u32 } else { ((sd.log2() * 8.0).round() as u32).min(0xff) };
    ...
    m.insert("CI".into(), if tcp_closed_ipids.is_empty() { "Z".into() } else { classify_ipid(tcp_closed_ipids) });
    m.insert("II".into(), if icmp_ipids.is_empty() { "O".into() } else { classify_ipid(icmp_ipids) });
...
fn build_ecn_test(reply: &Option<TcpReply>, probe_seq: u32) -> Option<HashMap<String, String>> {
    let mut m = HashMap::new();
    let Some(r) = reply else { m.insert("R".into(), "N".into()); return Some(m); };
...
fn build_t1_test(reply: &Option<TcpReply>, probe_seq: u32, probe_ack: u32) -> Option<HashMap<String, String>> {
    let mut m = HashMap::new();
    let Some(r) = reply else { m.insert("R".into(), "N".into()); return Some(m); };
    m.insert("R".into(), "Y".into());
    m.insert("DF".into(), if r.df { "Y" } else { "N" }.into());
...
fn build_tx_test(reply: &Option<TcpReply>, probe_seq: u32, probe_ack: u32) -> Option<HashMap<String, String>> {
    let mut m = HashMap::new();
    let Some(r) = reply else { m.insert("R".into(), "N".into()); return Some(m); };
    m.insert("R".into(), "Y".into());
    m.insert("DF".into(), if r.df { "Y" } else { "N" }.into());
...
fn build_u1_test(
    reply: &Option<IcmpUnreachReply>, sent_ip_id: u16, sent_udp_cksum: u16,
    sent_ip_total_len: u16, sent_data: &[u8],
    ...
    m.insert("RIPL".into(),
        if r.embedded_ip_total_len == sent_ip_total_len { "G".into() } else { hex_val(r.embedded_ip_total_len as u32) });
    m.insert("RID".into(),
        if r.embedded_ip_id == sent_ip_id { "G".into() } else { hex_val(r.embedded_ip_id as u32) });
    m.insert("RIPCK".into(),
        if r.embedded_ip_cksum == 0 { "Z".into() }
        else if r.embedded_ip_cksum == r.embedded_ip_cksum_computed { "G".into() }
        else { "I".into() });
    m.insert("RUCK".into(),
        if r.embedded_udp_cksum == 0 { "Z".into() }
        else if r.embedded_udp_cksum == sent_udp_cksum { "G".into() }
        else { "I".into() });
    let rud_ok = r.embedded_udp_data.len() >= sent_data.len().min(r.embedded_udp_data.len())
        && r.embedded_udp_data.starts_with(&sent_data[..sent_data.len().min(r.embedded_udp_data.len())]);
    m.insert("RUD".into(), if rud_ok && !r.embedded_udp_data.is_empty() { "G".into() } else { "I".into() });
    Some(m)
}

fn build_ie_test(ie1: &Option<IcmpEchoReply>, ie2: &Option<IcmpEchoReply>) -> Option<HashMap<String, String>> {
    let mut m = HashMap::new();
...
pub fn probe_ipv4_os(
    target: Ipv4Addr, open_tcp: u16, closed_tcp: u16, closed_udp: u16, probe_timeout: Duration,
) -> Result<SubjectFingerprint> {
    let (mut tcp_tx, mut tcp_rx) = transport_channel(
        65536, TransportChannelType::Layer3(IpNextHeaderProtocols::Tcp),
    ).context("raw TCP for OS detection — try sudo / capabilities")?;
    let (_icmp_tx, mut icmp_rx) = transport_channel(
        65536, TransportChannelType::Layer3(IpNextHeaderProtocols::Icmp),
    ...
    for i in 0..NUM_SEQ_SAMPLES {
        ...
        send_tcp_probe(&mut tcp_tx, src, target, sport, open_tcp,
            tcp_seq_base + i as u32, 0, 0, TcpFlags::SYN, PRB_WIN[i], 0, false, PRB_OPTS[i], 64, id)?;
        seq_replies[i] = recv_tcp_reply(&mut tcp_rx, target, sport, probe_timeout)?;
        if i + 1 < NUM_SEQ_SAMPLES { thread::sleep(Duration::from_millis(OS_SEQ_PROBE_DELAY_MS)); }
    }

    // ── Phase 2: ECN ──
    let ecn_sport = tcp_port_base + 6;
    send_tcp_probe(&mut tcp_tx, src, target, ecn_sport, open_tcp,
        tcp_seq_base, 0, 0, TcpFlags::SYN | TcpFlags::ECE | TcpFlags::CWR,
        PRB_WIN[6], 0, false, PRB_OPTS[6], 64, next_ipid(&mut ipid_ctr))?;
    ...
    let mut send_t = |idx: usize, dport: u16, flags: u8| -> io::Result<Option<TcpReply>> {
        ...
        let id = next_ipid(&mut ipid_ctr);
        send_tcp_probe(&mut tcp_tx, src, target, sport, dport,
            tcp_seq_base, tcp_ack, 0, flags, PRB_WIN[idx], 0, false, PRB_OPTS[idx], 64, id)?;
        recv_tcp_reply(&mut tcp_rx, target, sport, probe_timeout)
    };
    let t2_reply = send_t(7, open_tcp, 0)?;
    let t3_reply = send_t(8, open_tcp, TcpFlags::SYN | TcpFlags::FIN | TcpFlags::URG | TcpFlags::PSH)?;
    ...
    let t7_reply = send_t(12, closed_tcp, TcpFlags::FIN | TcpFlags::PSH | TcpFlags::URG)?;

    // ── Phase 5: U1 (UDP to closed port) ──
    let u1_data = [0x43u8; 300];
    let u1_ipid = next_ipid(&mut ipid_ctr);
    let u1_ip_total: u16 = 20 + 8 + 300;
    let u1_udp_ck = send_udp_probe(&mut tcp_tx, src, target, udp_port_base, closed_udp,
        &u1_data, 64, true, u1_ipid)?;
    let u1_reply = recv_icmp_unreachable(&mut icmp_rx, target, closed_udp, probe_timeout)?;

    // ── Phase 6: IE (2 ICMP echo requests) ──
    send_icmp_echo(&mut tcp_tx, src, target, 9, icmp_id, 295, 120, 64, true, 0,
    ...
    send_icmp_echo(&mut tcp_tx, src, target, 0, icmp_id + 1, 296, 150, 64, false, 4,
        next_ipid(&mut ipid_ctr))?;
    ...
    let good: Vec<usize> = (0..NUM_SEQ_SAMPLES).filter(|&i| seq_replies[i].is_some()).collect();
    if good.len() >= 4 {
        let tcp_open_ipids: Vec<u16> = good.iter().map(|&i| seq_replies[i].as_ref().unwrap().ipid).collect();
        let tcp_closed_ipids: Vec<u16> = [&t5_reply, &t6_reply, &t7_reply].iter()
            .filter_map(|r| r.as_ref().map(|t| t.ipid)).collect();
        let icmp_ipids: Vec<u16> = [&ie1, &ie2].iter()
            .filter_map(|r| r.as_ref().map(|e| e.ipid)).collect();
        let seq_nums: Vec<u32> = good.iter().map(|&i| seq_replies[i].as_ref().unwrap().seq).collect();
        let seq_times: Vec<Instant> = good.iter().map(|&i| seq_replies[i].as_ref().unwrap().recv_time).collect();
        let timestamps: Vec<u32> = good.iter().map(|&i| seq_replies[i].as_ref().unwrap().tsval).collect();

        // SEQ test
        subject.tests[0] = Some(build_seq_test(
            &seq_nums, &seq_times, &timestamps, &tcp_open_ipids, &tcp_closed_ipids, &icmp_ipids));

        // OPS test (O1-O6)
        let mut ops = HashMap::new();
        for (j, &i) in good.iter().enumerate().take(6) {
            ops.insert(format!("O{}", j + 1), seq_replies[i].as_ref().unwrap().options_fp.clone());
        }
        ...
        for (j, &i) in good.iter().enumerate().take(6) {
            win.insert(format!("W{}", j + 1), hex_val(seq_replies[i].as_ref().unwrap().window as u32));
        }
