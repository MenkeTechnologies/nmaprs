//! Raw SCTP scans (`-sY` INIT, `-sZ` COOKIE_ECHO) over IPv4 — CRC32c + pipelined recv like [`crate::syn`].
//!
//! **IPv4 only** (privileged). **IPv6 targets are skipped** with a warning. No `pnet` SCTP types:
//! we build headers manually (RFC 4960).
...
fn sctp_ipv4_one_round(
    ...
    for &(gidx, dst_ip, port) in subset {
        ...
        let pkt = build_sctp_probe(src_ip, dst_ip, sport, port, probe, &mut rng);
