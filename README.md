...
| SYN (`-sS`) | **Implemented** — raw IPv4 + **separate** raw IPv6 TCP path via `pnet` — **requires privileges**; batched sends + unified receive (indexed probes, ~Nmap-style parallelism); falls back to TCP connect per address family on failure |
