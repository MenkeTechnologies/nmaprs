...
| Evasion (`-g`, `--ttl`, `--badsum`, `-D`, `-S`, `--data*`, `-f`/`--mtu`) | **Implemented** — source port, TTL, bad checksum, decoys, source IP spoofing, custom data payload, and fragmentation MTU are **wired into raw TCP packet construction** (IPv4 SYN/NULL/FIN/Xmas/ACK/Window/Maimon) |
