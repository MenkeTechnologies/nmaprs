...
| Host discovery (before port scan) | **Implemented** — skipped with `-Pn` / `--no-ping`. **Default** runs **ICMP echo** and **TCP connects** to **443** and **80** **in parallel** (`tokio::join`) for wall-clock time (Nmap uses raw SYN/ACK; we treat connect success or `ECONNREFUSED` as “up”). Explicit `-PE` / `-PS` / `-PA` / `-PU` with optional port lists (`-PU` defaults to port **40125**; UDP reply or ICMP-unreachable-style socket errors → “up”; pure recv timeout → not up). `-PP`, `-PM`, `-PY`, `-PO` ignored with warning |
...
4. **Discovery** (`src/discovery.rs`) — before port scan (unless `-Pn`); default ICMP + TCP run concurrently; **TCP** and **UDP** `-PU` probes use `buffer_unordered(effective_probe_concurrency)` like port scans.
