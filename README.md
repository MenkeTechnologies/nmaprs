...
| Host discovery (before port scan) | **Implemented** — skipped with `-Pn` / `--no-ping`. **ARP ping** auto-runs for **local-subnet IPv4** hosts (disable with `--disable-arp-ping`); it has **no flag of its own** — nmap's `-PR` is **not** accepted. **Default** runs **raw ICMP echo** in parallel with **raw TCP SYN** to **443** and **80** (same engine as `-sS`); **RST/SYN-ACK** ⇒ up; falls back to **TCP connect** if raw fails. **`-PS`**: raw SYN; **`-PA`**: raw TCP **ACK** probes (RST/SYN-ACK), same sharded pipelines + connect fallback. **`-PU`** (default **40125**): UDP / ICMP-style socket errors. **`-PY`**: SCTP **INIT** (default port **80**), same raw SCTP engines as `-sY`; INIT-ACK/ABORT ⇒ up; **privileged**; IPv4/IPv6 merged independently on partial failure. **`-PO`**: IP protocol ping (default protocols **1,2,4**), same engines as `-sO`; host **up** if any probe is **open** or **closed**; **privileged**; IPv4/IPv6 merged independently on partial failure. **`-PP`** / **`-PM`**: ICMP **timestamp** and **address mask** requests (IPv4 only, **Unix** raw ICMP); **timestamp reply** / **mask reply** ⇒ up; **`connect_timeout`** applies to TCP/UDP/SCTP/IP-proto waits and to these ICMP probes; non-Unix builds skip `-PP`/`-PM` with a warning |
| IPv6 (`-6`) | **Implemented** — targets + scans (including raw SYN when privileged) |
| `-iL` / `-iR` | **Implemented** |
| `--resume` | **Implemented** — JSON checkpoint of completed `(host, port)`. The checkpoint is subtracted from the work list **before** scan dispatch (`src/lib.rs`), so it applies to **every port-scan kind**: TCP connect, UDP, raw half-open TCP, **SCTP**, **IP protocol (`-sO`)**, **idle (`-sI`)** and **FTP bounce (`-b`)** — remaining pairs only |
...
| Port specs (`-p`, `-F`, `--top-ports`, `--port-ratio`, `-r`) | **Implemented** — embedded TCP frequency list (`data/top_ports.txt`); **`-p -`** shorthand for all 65536 ports; **`-F`** = top 100, no `-p` = top 1000; **`--port-ratio R`** takes the leading `round(R × table length)` entries of that ranked list (`R` in `(0,1]`); ports are **shuffled** by default, **`-r`** scans them sequentially |
| Target exclusion (`--exclude`, `--excludefile`) | **Implemented** — comma-separated targets / one spec per line, subtracted from the expanded host set (same grammar as positional targets: IPs, CIDR, nmap-style ranges, hostnames) |
| `--open` / `--reason` | **Implemented** — `--open` prints only `open` ports; `--reason` appends the state reason to every stdout and `-oN` port line |
| Evasion (`-g`, `--badsum`, `-D`, `-S`, `--data*`, `--spoof-mac`) | **Implemented** — source port, bad checksum, decoys, source IP spoofing and custom data payload are **wired into raw TCP packet construction** (IPv4 SYN/NULL/FIN/Xmas/ACK/Window/Maimon); **`--spoof-mac`** applies to ARP discovery frames |
| `--ttl`, `-f` / `--mtu`, `--ip-options`, `-e`, `--send-eth` / `--send-ip`, `--packet-trace`, `--system-dns` | **Parsed only** — accepted and validated (e.g. `--mtu` must be a multiple of 8) so nmap command lines run unchanged, but **not yet applied** to the emitted packets or to interface/DNS selection |
...
## [0x05] SCANNING DATA STREAMS // EXAMPLES
...
## [0x07] BENCHMARKS // PERFORMANCE MATRIX

 ┌──────────────────────────────────────────────────────────────┐
 │ BENCHMARK: nmaprs vs nmap // WALL CLOCK COMBAT              │
 └──────────────────────────────────────────────────────────────┘
...
## [0x08] ARCHITECTURE // SYSTEM INTERNALS

```
 ┌─────────────────────────────────────────────────────────────┐
 │  [1] ARGV EXPAND      ▓▓  normalize glued nmap tokens      │
 │  [2] PLAN             ▓▓  ScanPlan + optional --scanflags   │
 │  [3] TARGETS          ▓▓  IPv4/6, CIDR, DNS, -iL, -iR      │
 │  [4] DISCOVERY        ▓▓  ICMP + raw SYN + ARP + fallbacks  │
 │  [5] SCAN ENGINE      ▓▓  worker-pool, sharded pipelines    │
 │  [6] POST-SCAN        ▓▓  ping, trace, resume, NSE, OS, sV  │
 │  [7] OUTPUT           ▓▓  -oN, -oG, -oX, -oS, -oM, -oH     │
 └─────────────────────────────────────────────────────────────┘
```
...
4. **Discovery** (`src/discovery.rs`, `src/icmp_ping.rs`) — before port scan (unless `-Pn`); default ICMP + raw SYN (`syn.rs`) run concurrently; `-PS` raw SYN, `-PA` raw ACK, `-PU` UDP, `-PY` SCTP, `-PO` IP protocol (`ip_proto.rs`), `-PP`/`-PM` legacy ICMP on Unix (`icmp_ping.rs`); `connect_timeout` from `--min-rtt-timeout` / timing template.
...
```
░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
░░ >>> JACK IN. SCAN THE GRID. OWN EVERY PORT. <<<         ░░
░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
```
