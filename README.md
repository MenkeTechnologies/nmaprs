...
| SCTP (`-sY` INIT, `-sZ` COOKIE_ECHO) | **Implemented (IPv4)** — raw IP + SCTP (CRC32c), pipelined recv + up to **16** shards like SYN; INIT-ACK / COOKIE-ACK ⇒ `open`, ABORT ⇒ `closed`; **IPv6 skipped** with warning; **privileged** raw socket |
| Idle scan (`-sI`) | **Not implemented** — IP-ID zombie prediction; use **[Nmap](https://nmap.org/)** |
...
| `--max-retries` | **Implemented** — extra attempts after **probe timeout** (total tries = `1 + N`) for TCP connect, UDP, raw half-open TCP, **FTP bounce**, and **SCTP** (retry rounds omit scan-delay / rate pacer like TCP retries) |
...
5. **Scan** (`src/scan.rs`, `src/syn.rs`, `src/sctp.rs`, `src/ip_proto.rs`, `src/ftp_bounce.rs`, `src/icmp_listen.rs`, `src/ipv6_l4.rs`) — optional `--min-hostgroup` / `--max-hostgroup` batching in `src/lib.rs` (`host_batches`); UDP ICMP listeners are **one session per scan** (shared `DashMap` across batches). TCP connect / UDP / ping use `futures::stream` + `buffer_unordered(effective concurrency)` (single cap; no duplicate semaphores) / raw IPv4 + IPv6 half-open TCP + **SCTP** (`-sY`/`-sZ`, CRC32c, sharded blocking pool) + **IPv4 IP protocol** (`-sO`, ICMP) + **FTP bounce** (`-b`, Tokio parallel FTP sessions).
