...
| Host discovery (before port scan) | **Implemented** — skipped with `-Pn` / `--no-ping`. **Default** runs **ICMP echo** in parallel with **raw TCP SYN** to **443** and **80** via the same stack as `-sS` (mixed IPv4/IPv6 `tokio::join` + sharded pipelines); **RST/SYN-ACK** ⇒ up. Falls back to **TCP connect** if raw sockets fail (e.g. unprivileged). **`-PS`** uses raw SYN the same way; **`-PA`** uses **TCP connect** (raw ACK not implemented). **`-PU`** (default port **40125**): UDP reply or ICMP-unreachable-style socket errors ⇒ up. TCP connect / UDP waits use **`connect_timeout`** from RTT options. `-PP`, `-PM`, `-PY`, `-PO` ignored with warning |
